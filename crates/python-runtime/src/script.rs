        ...
        Python::attach(|py| {
            ...
            let stdio = PyModule::import(py, intern!(py, "sys"))
                ...
                .and_then(|sys| {
                    ...
                    #[cfg(target_os = "wasi")]
                    {
                        // CPython 3.15 freezes the `encodings` package; its
                        // frozen __path__ points at an
                        // on-disk stdlib directory that does
                        // not exist in this zip-only layout, breaking every
                        // non-builtin codec (zipfile itself needs cp437 to read
                        // a central directory, e.g. for
                        // importlib.metadata lookups
                        // against bundle.zip). Repoint __path__ at the stdlib
                        // zip before importing the
                        // HTTPX2 sandbox transport.
                        py.run(
                            c"
