...
fn main() {
    ...
    if target.starts_with("wasm32") {
        // Use PIC-compiled WASI libs from WASI_PYTHON_DEV (shared with the
        // Python runtime build environment). The Rust sysroot's libc.a
        // is not compiled with -fPIC, so we must link against the WASI
        // SDK's libc.so instead.
