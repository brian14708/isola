...
impl<T: HostView> Host for HostImpl<T> {
    async fn connect(
        ...
        let uri: http::Uri = conn.0.url.parse().map_err(|e| {
            ErrorCode::ProtocolError(Some(format!("invalid websocket uri '{}': {e}", conn.0.url)))
        })?;

        let meta = crate::net::WebsocketMeta { uri: uri.clone() };
        if let Err(reason) = self.0.network_policy().check_websocket(&meta).await {
            event!(
                name: "net.deny",
            ...
            return Ok(self.0.table().push(FutureWebsocket::Ready(Err(
                ErrorCode::ProtocolError(Some(reason)),
            )))?);
        }
        ...
        let s = wasmtime_wasi::runtime::spawn({
            async move {
                ...
                let outbound: Pin<Box<dyn Stream<Item = Message> + Send + Sync>> =
                    Box::pin(tokio_stream::wrappers::ReceiverStream::new(write_rx));

                let mut headers = http::HeaderMap::new();
                if let Some(raw_headers) = &conn.0.headers {
                ...
                let req = crate::WebsocketRequest {
                    uri,
                    headers,
                    outbound,
                };
                ...
                let mut response = if let Some(timeout) = conn.0.timeout {
                    tokio::time::timeout(timeout, host.websocket_connect(req))
                        .await
                        .map_err(|_| ErrorCode::Timeout)?
                } else {
                    host.websocket_connect(req).await
                }
                .map_err(|e| ErrorCode::ConnectionFailed(Some(e.to_string())))?;

                if response.status().is_redirection() {
                    return Err(ErrorCode::ProtocolError(Some(format!(
                        "websocket redirect status: {}",
                        response.status()
                    ))));
                }
                ...
                let mapped_stream = response_stream
                    .map(|result| result.map_err(|e| anyhow::Error::msg(e.to_string())));
